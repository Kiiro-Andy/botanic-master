...
export default function PlantDetailScreen({ route }) {
  ...
  return (
    <ScrollView style={styles.container}>
	  ...
	  <TouchableOpacity onPress={() => toggleFavorite(plant)} style={{ padding: 10 }}>
		<Text style={{ fontSize: 18}}>
			{isFavorite(plant.id) ?  "💔 Quitar de favoritos" : "❤️ Agregar a favoritos"}
		</Text>
	  </TouchableOpacity>
      ...
      ? `Nivel de humedad necesaria: ${main_species.growth.atmospheric_humidity}/10 (${interpretLevel(main_species.growth.atmospheric_humidity)})`
      : "Mantener suelo húmedo"}</Text>
        <Text>☀️ Luz: {" "}
    {main_species.growth?.light != null
      ? `Nivel de cantidad de sol necesaria: ${main_species.growth.light}/10 (${interpretLevel(main_species.growth.light)})`
      : "Soleado o semisombra"}</Text>
        <Text>
          🌡️ Temperatura:{" "}
    ...
    {main_species.growth?.soil_nutriments != null
      ? `Nivel de nutrientes: ${main_species.growth.soil_nutriments}/10 (${interpretLevel(main_species.growth.soil_nutriments)})`
      : "Bien drenado y fértil"}</Text>
      </View>
      ...
      <View style={styles.section}>
        ...
        {details.distribution?.native?.length > 0 && (
          <Text>🌍 Nativa de: {details.distribution.native.join(", ")} || "N/A" </Text>
        )}
        {details.flowering_months && (
          <Text>🌸 Época de floración: {details.flowering_months} || "N/A" </Text>
        )}
